...
function generateDeviceId(deviceIdStruct) {
  ...
  const default_rx = new RegExp("[^A-Za-z0-9_]", "g");
  function esc(str, rx = default_rx) {
    return str.replace(rx, chr => {
  ...
  if (deviceIdStruct["ProductClass"]) {
    return (
      ...
      "-" +
      esc(deviceIdStruct["ProductClass"], new RegExp("[^A-Za-z0-9_\(\)\ ]", "g")) +
      "-" +
